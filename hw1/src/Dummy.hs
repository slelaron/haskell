...
mergeSort :: Ord a => [a] -> [a]
mergeSort [] = []
mergeSort array = internalSort $ map (: []) array
  where
    merge :: Ord a => [a] -> [a] -> [a]
    merge [] ys             = ys
    merge xs []             = xs
    ...
    internalSort x = case len of
         1 -> head x
         _ -> internalSort $ map (uncurry merge) $ uncurry zip $ splitAt (div (len + 1) 2)
                 $ if even len 
                   then x 
                   else [] : x
      where
...
buildHouse town@Town { houses = (house NE.:| another) } citizens
    = town { houses = (citizens NE.:| (house : another)) }
...
eraseInTree need = internal
  where
    ...
    internal (Node args@(key NE.:| keys) left right)
        ...
        | not (null keys)  = Node (key NE.:| tail keys) left right
